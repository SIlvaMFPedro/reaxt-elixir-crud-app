defmodule ReaxtElixirCrudApp.App do
  use Application
  def start(_type, _args), do: Supervisor.start_link(ReaxtElixirCrudApp.App.Sup,nil)

  defmodule Sup do
    use Supervisor
    def init(nil), do: supervise([
      Plug.Adapters.Cowboy.child_spec(:http,ReaxtElixirCrudApp,[], port: 4000)
    ], strategy: :one_for_one)
  end
end

defmodule ReaxtElixirCrudApp do
  use Plug.Router
  if Mix.env == :dev do 
    use Plug.Debugger
    plug WebPack.Plug.Static, at: "/public", from: :reaxt_elixir_crud_app
  else
    plug Plug.Static, at: "/public", from: :reaxt_elixir_crud_app
  end
  plug :match
  plug :dispatch
